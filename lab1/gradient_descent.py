...
def repeat_dataframe_for_points(points_array, parameter_array, parameter_index, step_length, D, folder_name, function):
    iteration = 1
    for start_point in points_array:
        data = generate_dataframe(parameter_array, parameter_index, start_point, step_length, D)
        data.to_csv(f"function_{function}/{folder_name}/{iteration}_{function}.csv")
        iteration += 1
...
def run_tests_function_f(start, step_length):
    ...
    repeat_dataframe_for_points(start_points, step_sizes, 1, step_length, D, "step_length", "f")

    # iterations limit
    limits = np.arange(5, 200, 5)
    repeat_dataframe_for_points(start_points, limits, 2, step_length, D, "iterations_limit", "f")

    return data


print(run_tests_function_f(0, 0.3))
